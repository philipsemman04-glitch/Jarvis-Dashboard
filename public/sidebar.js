...
(function () {
  ...
  function buildSidebarHTML(workspaces) {
    ...
    function itemHTML(w) {
      ...
      const href = sharedBusiness.includes(canonical)
      ...
      return `<a class="sb-item ${isActive(href) ? "active" : ""} ${comingSoon ? "coming-soon" : ""}" href="${href}">
        <span class="sb-icon">${w.icon || "📁"}</span><span>${w.displayName || w.name}</span>
      </a>`;
    }
  ...
  const FALLBACK = [
    ...
    { name: "One Night Guest", icon: "🏛️", route: "/project-command-center.html?project=One%20Night%20Guest", type: "Business", status: "Live", order: 1 },
