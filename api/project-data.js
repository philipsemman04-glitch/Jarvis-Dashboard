...
const NOTION_PAGE_URL_BY_PROJECT = { "RoutePup": "https://app.notion.com/p/RoutePup-3b664152b99e81218f44c13452dc54d2" };
...
// Notion returns at most 100 pages per query. Command-center period totals
// must use the complete database, otherwise older/newer dated tasks can be
// silently omitted when the database grows past the first response page.
async function notionQueryAll(dataSourceId, body = {}) {
  const results = [];
  let start_cursor;
  do {
    const page = await notionQuery(dataSourceId, { ...body, ...(start_cursor ? { start_cursor } : {}) });
    results.push(...(page.results || []));
    start_cursor = page.has_more ? page.next_cursor : null;
  } while (start_cursor);
  return { results };
}

async function notionCreatePage(dataSourceId, properties) {
  // Notion's newer multi-source database model means the ID this codebase
  // uses everywhere (the one that works for querying) is a data source ID,
  // not a database ID — the create-page endpoint needs to be told that
  // explicitly via parent.data_source_id, or it 404s even when the
  // integration has full access.
...
function multi(prop) { return (prop?.multi_select || []).map(x => x.name); }
...
// ---- Area management: create/rename/archive, so a project's board
// structure is editable by Aurelio himself, not something only Philips
// can change in code. Archiving (not deleting) keeps history intact and
// just drops the area off the live board via the Status=Inactive filter
// already applied above.
async function handleCreateArea(req, res) {
  const { project, name } = req.body || {};
  if (!project || !name || !name.trim()) { res.status(400).json({ error: "project and name are required" }); return; }
  const page = await notionCreatePage(DB_ENTITIES, {
    "Entity Name": { title: [{ text: { content: name.trim() } }] },
    "Empresa": { select: { name: project } },
    "Status": { select: { name: "Active" } },
  });
  res.status(200).json({ ok: true, areaId: page.id });
}

async function handleRenameArea(req, res) {
  const { areaId, name } = req.body || {};
  if (!areaId || !name || !name.trim()) { res.status(400).json({ error: "areaId and name are required" }); return; }
  await notionUpdatePage(areaId, { "Entity Name": { title: [{ text: { content: name.trim() } }] } });
  res.status(200).json({ ok: true });
}

async function handleArchiveArea(req, res) {
  const { areaId } = req.body || {};
  if (!areaId) { res.status(400).json({ error: "areaId is required" }); return; }
  await notionUpdatePage(areaId, { "Status": { select: { name: "Inactive" } } });
  res.status(200).json({ ok: true });
}

async function handleCreateTask(req, res) {
  const b = req.body || {};
  if (!b.taskName || !String(b.taskName).trim()) { res.status(400).json({ error: "taskName is required" }); return; }
  const properties = {
...
module.exports = async (req, res) => {
  // Always fetch fresh from Notion — without this, GET requests can get
  // served a stale cached response (confirmed happening: server logs show
  // 304s on /api/project-data), which is exactly why a task that's
  ...
  try {
    if (req.method === "POST") {
      ...
      if (action === "create-area") return await handleCreateArea(req, res);
      if (action === "rename-area") return await handleRenameArea(req, res);
      if (action === "archive-area") return await handleArchiveArea(req, res);
      if (action === "create-task") return await handleCreateTask(req, res);
      res.status(400).json({ error: "Unknown or missing action" });
      return;
    }

    const key = slugify(req.query?.project || "");
    if (!key) {
    ...
    const [projectsRes, workspacesRes] = await Promise.all([
      notionQuery(DB_PROJECTS, {}),
      notionQuery(DB_WORKSPACES, {}).catch(() => ({ results: [] })),
    ]);
    ...
    const workspaceMatch = (workspacesRes.results || []).find(p => text(p.properties["Name"]) === projectName);
    const configuredDisplayName = workspaceMatch ? text(workspaceMatch.properties["Display Name"]) : "";
    const displayName = configuredDisplayName && !/^https?:\/\//i.test(configuredDisplayName) && !/amazonaws\.com|X-Amz-Signature|X-Amz-Algorithm/i.test(configuredDisplayName)
      ? configuredDisplayName
      : projectName;
    const workspaceIconFile = workspaceMatch?.properties["Icon Image"]?.files?.[0]?.file?.url || workspaceMatch?.properties["Icon Image"]?.files?.[0]?.external?.url;
    const icon = workspaceMatch ? (workspaceIconFile || text(workspaceMatch.properties["Display Icon"]) || text(workspaceMatch.properties["Icon"]) || ICON_BY_PROJECT[projectName] || "🗂️") : (ICON_BY_PROJECT[projectName] || "🗂️");
    const workspaceCoverFile = workspaceMatch?.properties["Cover Image"]?.files?.[0]?.file?.url || workspaceMatch?.properties["Cover Image"]?.files?.[0]?.external?.url;

    const [entitiesRes, actionsRes] = await Promise.all([
      notionQueryAll(DB_ENTITIES, {
        filter: { property: "Empresa", select: { equals: projectName } },
        sorts: [{ property: "Entity Name", direction: "ascending" }],
      }),
      notionQueryAll(DB_ACTIONS, {
    ...
    })).filter(a => a.status !== "Inactive"); // archived areas stay in Notion but drop off the live board
    ...
    const tasks = (actionsRes.results || []).map(p => {
      ...
      return {
        id: p.id,
        notionUrl: p.url,
        taskName: text(p.properties["Task Name"]),
        name: text(p.properties["Task Name"]), // kept for the older flat list view
        status: select(p.properties["Status"]),
        ...
        attachments: fileList(p.properties["Attachments"]),
    ...
    let cards = [];
    if (cardsDb) {
      const cardsRes = await notionQueryAll(cardsDb, {});
    ...
    // Real kanban board grouped by area — the same structure ONG's board
    // uses, generalized so every project gets the identical system instead
    // of a separate, more limited page. Includes areas with zero open tasks
    // (empty, honest) so newly-created areas show up immediately.
    const CLOSED_STATUSES = ["Terminado", "Cancelado"];
    const openTasks = tasks.filter(t => !CLOSED_STATUSES.includes(t.status));
    const board = areas.map(a => ({ area: a.name, areaId: a.id, tasks: openTasks.filter(t => t.areaId === a.id) }));
    board.push({ area: "Sin área", areaId: null, tasks: openTasks.filter(t => !t.areaId) });

    res.status(200).json({
      project: { key, canonicalName: projectName, label: displayName, notionUrl: NOTION_PAGE_URL_BY_PROJECT[projectName] || match.url || null, icon, coverUrl: workspaceCoverFile || workspaceMatch?.properties["Cover URL"]?.url || workspaceMatch?.properties["Cover"]?.url || match.properties["Cover URL"]?.url || match.properties["Cover"]?.url || null },
      areas,
      board,
      tasks,
      stats: {
