...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const icon = workspaceMatch ? (text(workspaceMatch.properties["Display Icon"]) || text(workspaceMatch.properties["Icon"]) || ICON_BY_PROJECT[projectName] || "🗂️") : (ICON_BY_PROJECT[projectName] || "🗂️");
    ...
    res.status(200).json({
      project: { key, canonicalName: projectName, label: displayName, icon, coverUrl: workspaceMatch?.properties["Cover URL"]?.url || workspaceMatch?.properties["Cover"]?.url || match.properties["Cover URL"]?.url || match.properties["Cover"]?.url || null },
      areas,
      board,
      tasks,
      stats: {
