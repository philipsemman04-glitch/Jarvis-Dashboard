...
async function handleCalendar(req, res) {
  const DB_MEETINGS_DATA_SOURCE = "ad03f382-6e63-4e86-b905-32772e16600c";
...
async function handleDecisionUpdate(req, res) {
  const { pageId, status, impact, context, finalDecision } = req.body || {};
  if (!pageId) { res.status(400).json({ error: "pageId is required" }); return; }
  const properties = {};
...
async function handleMeetingsList(req, res) {
  ...
  const meetings = pages.map((p) => {
    ...
    return {
      ...
      decisions: text(props["Decisions"]), notes: text(props["Notes"]),
    };
  });
...
async function handleMeetingUpdate(req, res) {
  const { pageId, status, summary, decisions, notes } = req.body || {};
  if (!pageId) { res.status(400).json({ error: "pageId is required" }); return; }
  const properties = {};
