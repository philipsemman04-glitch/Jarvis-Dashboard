...
async function handleUpload(req, res) {
  ...
  const MAX_BYTES = 3 * 1024 * 1024; // same ~4.5MB Vercel payload cap as task-api.js's upload
  const buffer = Buffer.from(dataBase64, "base64");
  if (buffer.length > MAX_BYTES) { res.status(400).json({ error: "Archivo demasiado grande — el límite actual es 3MB." }); return; }

  const createRes = await fetch("https://api.notion.com/v1/file_uploads", { method: "POST", headers: headers(), body: JSON.stringify({}) });
  ...
  const r = await fetch(`https://api.notion.com/v1/pages/${pageId}`, {
    method: "PATCH", headers: headers(),
    body: JSON.stringify({ properties: { Attachments: { files: [...existingRaw, { type: "file_upload", file_upload: { id: upload.id }, name: filename }] } } }),
  });
  if (!r.ok) throw new Error(`Notion update failed (${r.status}): ${await r.text()}`);
